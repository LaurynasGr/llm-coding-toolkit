...
const COMMAND_NAMES = ['review-comments', 'prs', 'add-token', 'list-tokens', 'autocomplete'];
...
function buildBashOrZshSnippet(shell: 'bash' | 'zsh'): string {
  const words = COMMAND_NAMES.join(' ');
...
compdef _llmct_completions llmct llmct-dev
${END_MARKER}
`;
  }
...
complete -F _llmct_completions llmct
${END_MARKER}
`;
}
